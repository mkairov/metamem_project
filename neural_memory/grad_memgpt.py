...
class GradMemGPT(PreTrainedModel):
    ...
    def __init__(self, config, n_mem_tokens=8, K=3, lr=2e-02, use_adam=True, grad_mode="none", n_ctrl_tokens=0,
                 inner_clip_value=None, inner_clip_norm=None, n_mem_iters: int = 1):
        """
        ...
        self.neural_mem = NeuralMemory(n_mem_tokens, n_embd, n_heads=4, n_iters=n_mem_iters, cross_attention=True)
        for p in self.neural_mem.parameters():
    ...
    def forward(self, input_ids, labels=None, return_mem=False):
        ...
        if self.K and context_input_ids.ne(pad_id).any():
            ...
            with torch.enable_grad():
                ...
                nm_named_params = list(self.neural_mem.named_parameters())
                nm_params = [p.detach().clone().requires_grad_(True) for _, p in nm_named_params]
                nm_opt_states = [{} for _ in nm_params]

                for inner_step in range(self.K):
                    nm_param_dict = {name: p for (name, _), p in zip(nm_named_params, nm_params)}

                    effective_mem = mem_batch
                    refined_slots = functional_call(self.neural_mem, nm_param_dict, (effective_mem, ctx_emb)) # [B,M,d]
                    x_ctx = torch.cat([refined_slots, ctx_emb], dim=1) # [B,M+S,d]
                    ...
                    new_nm_params = []
                    for p, grad_p, state_p in zip(nm_params, grads_nm, nm_opt_states):
                        if self.use_adam:
                            new_p = self._adam_step(p, grad_p, state_p, inner_step + 1, self.lr)
                        else:
                            new_p = self._sgd_step(p, grad_p)
                        new_nm_params.append(new_p.detach().requires_grad_(True))
                    nm_params = new_nm_params
        ...
        effective_final_mem = mem_batch.detach()
        if self.K:
            nm_param_dict_final = {name: p.detach() for (name, _), p in zip(nm_named_params, nm_params)}
            refined_read_slots = functional_call(self.neural_mem, nm_param_dict_final, (effective_final_mem,))
        else:
            refined_read_slots = self.neural_mem(effective_final_mem)
        x_qry = torch.cat([refined_read_slots, qry_emb], dim=1)
...
class NeuralMemory(nn.Module):
    def __init__(self, n_slots: int, dim: int, n_heads: int=4, n_iters: int=1, cross_attention: bool = False):
        super().__init__()
        self.n_iters = n_iters
        self.n_slots = n_slots
        self.dim = dim # slot dim
    ...
    def _split_heads(self, x: torch.Tensor):
        ...
        H = self.n_heads
        dh = D // H
        return x.view(B, L, H, dh).transpose(1, 2) # (B,H,L,dh)

    def _merge_heads(self, x: torch.Tensor):
        B, H, L, dh = x.shape
        return x.transpose(1, 2).contiguous().view(B, L, H*dh)

    def forward(self, slots: torch.Tensor, context_emb: torch.Tensor | None = None):
        B = slots.size(0)

        if context_emb is not None and self.cross_attention:
            q = self._split_heads(self.to_q(slots))
            k = self._split_heads(self.to_k(context_emb))
            v = self._split_heads(self.to_v(context_emb))

            attn_out = self._attn(q, k, v)
            attn_out = self._merge_heads(attn_out)
            slots = slots + self.proj_out(attn_out)

            context_emb = None

        all_slots = slots if context_emb is None else torch.cat([slots, context_emb], dim=1)

        for _ in range(self.n_iters):
            q = self._split_heads(self.to_q(all_slots))
            k = self._split_heads(self.to_k(all_slots))
            v = self._split_heads(self.to_v(all_slots))

            attn_out = self._attn(q, k, v)
            attn_out = self._merge_heads(attn_out)
            all_slots = all_slots + self.proj_out(attn_out)

        return all_slots[:, : self.n_slots, :]
...
class MemoryCell(torch.nn.Module):
    def __init__(self, base_model, num_mem_tokens, init_inner_lr=1.0, init_stability_coef=0.02, inner_steps=2, **kwargs):
        super().__init__()
    ...
    def inner_loop(self, input_ids, **seg_kwargs):
        ...
        for _ in range(self.inner_steps):
            ...
            # preserve out with unmodified memory
            if first_out is None:
                first_out = out
            
            logits = out.logits[:, self.num_mem_tokens:]
            # logger.info(f"emb: {ctx_emb.shape}, mem: {mem_live.shape}, logits: {logits.shape}; labels: {lm_labels.shape}")
            # inner_loss = F.cross_entropy(logits[:, :-1], lm_labels[:, 1:])
            inner_loss = F.cross_entropy(logits[:, :-1].reshape(-1, logits.size(-1)), lm_labels[:, 1:].reshape(-1),)
            diff_loss = F.mse_loss(mem_live, memory_state)
            total_loss = inner_loss + torch.exp(self.log_stability_coef) * diff_loss
            g = torch.autograd.grad(total_loss, mem_live, create_graph=True)[0]
            g_norm = g.reshape(B, -1).norm(dim=1).detach()

            inner_loop_stats['inner_grad_norm_mean'] += g_norm.mean()
            inner_loop_stats['inner_recon_loss'] = inner_loss
            inner_loop_stats['inner_diff_loss'] = diff_loss
        ...
        inner_loop_stats['inner_final_loss'] = total_loss

        return first_out, mem_live, inner_loop_stats
...
class RecurrentWrapper(torch.nn.Module):
    ...
    def process_outputs(self, cell_outputs, **kwargs):
        ...
        out['inner_stability_coef'] = torch.exp(self.memory_cell.log_stability_coef).item()
